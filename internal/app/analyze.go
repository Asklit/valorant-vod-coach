...
func (BaselineObservationAnalyzer) AnalyzeObservations(ctx context.Context, request ObservationRequest) (ObservationResult, error) {
	...
	findings := []domain.Finding{
		...
		},
		{
			...
			Title:          "Gameplay analyzer is not configured",
			Detail:         "This compatibility fallback validates media ingestion only and does not inspect gameplay context.",
			Recommendation: "Use the local gameplay analyzer for CPU CV/OCR evidence selection, or explicitly configure an evaluated analyzer adapter.",
			Confidence:     1,
